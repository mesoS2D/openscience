...
def interpolate_lev_to_z_ncol(ds, zlimL = 0, zlimU = 135, dz=2.5): 
    """
    Takes in ds, which must have some data, and z (altitude) as a variable.
    
    Also takes in zlimL (default 0), zlimU (default 135) and dz (default 2.5).
    
    It looks over the datasets dimensions, namely, ncol and time, and interpolates each column onto a uniform vertical grid.
    """
    # PARTIALLY DONE, #TODO NEEDS IMPLEMENTATION OF DUMMY DIMENSIONS IF NOT IN DATASET
    # This might be absorbed into a general function interpolate_lev_to_z
    # along with the code from interpolate_lev_to_z_latlon.
    ...
    z_interp = np.arange(zlimU,zlimL-dz,-dz) # Create vertical coordinate in z (zlimL includes -dz so that z_interp finishes at 0)
    ds_interp_icol_step = []
...
def interpolate_lev_to_z_latlon(ds, zlimL = 0, zlimU = 135, dz=2.5): # In Progress
