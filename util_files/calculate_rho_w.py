...
def calculate_rho(T,pressure):
    """
    Calculate mass density in kg m^-3 from temperature (K) and pressure (hPa)
    """
...
def calculate_n(T,pressure): 
    """
    Calculate number density in particles cm^-3 from temperature (K) and pressure (hPa)
    """
    ...
    to_Pa = 100 # convert from hPa to Pa. Pa = kg m^-1 s^-2
    # n = pressure*to_Pa/(to_cm3*k*T) # Units: kg m^-1 s^-2 * kg^-1 m^-2 s^2 K * K^-1 = m^-3 | m^-3 * 1e-6 = cm^-3
    n = pressure*to_Pa/(k*T) # Units: kg m^-1 s^-2 * kg^-1 m^-2 s^2 K * K^-1 = m^-3 
